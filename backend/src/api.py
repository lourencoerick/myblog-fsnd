...
def get_articles():
    ...
    return (
        jsonify(
            {"sucess": True, "articles": [article.response() for article in articles]}
        ),
        200,
    )
...
def get_collections():
    ...
    return (
        jsonify(
            {
                "sucess": True,
                "collections": [collection.response() for collection in collections],
            }
        ),
        200,
    )
...
def request_malformed_authorization(error):
    description = getattr(error, "description", "Request lacks authorization parameter")
